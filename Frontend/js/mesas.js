...
async function recargarMesas() {
  ...
  if (contenedor)
    contenedor.innerHTML = `<div class="col-span-full rounded-3xl bg-white p-8 text-center font-bold text-slate-500">Cargando mesas desde la BD...</div>`;
  try {
    await cargarMesas();
    ...
    if (contenedor)
      contenedor.innerHTML = `<div class="col-span-full rounded-3xl border border-red-200 bg-red-50 p-8 text-center text-red-700"><h2 class="text-2xl font-black">No se pudo cargar mesas</h2><p class="mt-2 text-sm font-semibold">${escapeHtml(error.message)}</p></div>`;
  }
}

function renderEstadisticasMesas() {
  const contar = (estado) =>
    mesasBase.filter((m) => m.estado === estado).length;
...
function notificacionesDeMesa(mesa) {
  ...
  if (solicitudCuenta) {
    notificaciones.push({
      ...
      detalle:
        solicitudCuenta.nota ||
        "El cliente solicito la cuenta. Caja recibio la solicitud para gestionar el cobro.",
      color: "blue",
    });
  }
...
function botonAtenderNotificacion(notificacion, modo = "oscuro") {
  ...
  if (notificacion.tipo === "cuenta") {
    return `<span class="rounded-xl bg-blue-100 px-3 py-2 text-xs font-black text-blue-700">Enviada a Caja</span>`;
  }
  const accion = `atenderComentarioMesa('${escapeHtml(String(notificacion.id))}')`;
      ...
      ${pedidosLlevarListos
        .map((pedido) => {
          const codigo =
            ...
            pedido.codigo_llevar ||
            pedido.codigo ||
            `LLEV-${String(pedido.id_pedido || pedido.id || "").padStart(3, "0")}`;
          return `
...
function renderMesas() {
  ...
  const mesas =
    filtroMesas === "todas"
      ? mesasBase
      : mesasBase.filter((m) => m.estado === filtroMesas);
  if (!mesas.length) {
  ...
  contenedor.innerHTML = mesas
    .map((mesa) => {
      ...
      const alertaHtml = notificaciones.length
        ? notificaciones
            .map(
              (alerta) => `
...
async function atenderComentarioMesa(idComentario) {
  ...
  try {
    await apiJson(
      `/mesas/comentarios/${encodeURIComponent(idComentario)}/atender`,
...
async function atenderSolicitudCuenta(idSolicitud) {
  ...
  try {
    await apiJson(
      `/mesas/solicitudes-cuenta/${encodeURIComponent(idSolicitud)}/atender`,
      { method: "PATCH", body: JSON.stringify({}) },
    );
    await recargarMesas();
  } catch (error) {
    alert(
      `No se pudo marcar la solicitud de cuenta como atendida: ${error.message}`,
    );
  }
}
...
async function restaurarMesasDemo() {
  alert(
    "Las mesas vienen de la base de datos. Caja libera la cuenta cuando registra el pago completo.",
  );
}
